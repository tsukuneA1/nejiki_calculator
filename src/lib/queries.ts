...
} satisfies Prisma.BattleFactoryPokemonSelect;

type FactoryPokemonRecord = Prisma.BattleFactoryPokemonGetPayload<{
  select: typeof factoryPokemonSelect;
}>;

const toFactoryPokemon = (fp: FactoryPokemonRecord): FactoryPokemon => ({
  id: fp.id,
  hp: fp.hpEv,
  attack: fp.attackEv,
...
export async function getFactoryPokemons(): Promise<FactoryPokemon[]> {
  const data = await prisma.battleFactoryPokemon.findMany({
    select: factoryPokemonSelect,
  });

  return data.map(toFactoryPokemon);
}
...
export async function getAbilities(): Promise<string[]> {
  ...
  const abilities = new Set(
    data.flatMap(({ ability1, ability2 }) =>
      ability2 ? [ability1.name, ability2.name] : [ability1.name],
    ),
  );

  return ["なし", ...[...abilities].sort((a, b) => a.localeCompare(b, "ja"))];
}
